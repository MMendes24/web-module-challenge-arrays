...
function is31Flavors(flavors){
    if (flavors.length === 31){
        console.log("Woah! So many flavors!")
        return true;
    } else {
        console.log("Who took all of the flavors!")
        return false;
    }
...
function addFlavor(flavor, flavorList){
    flavorList.unshift("Rainbow Sherbert");
...
function getFlavorByIndex(flavorList, index){
    return console.log(flavorList[index]);
...
function removeFlavorByName(flavorList,flavor){
    ...
    flavorList.splice(flavorList.indexOf(flavor), 1);
...
function copy(arr, newarr){
    newarr = [...arr];
    newarr.unshift("the sacred test!");
    return newarr;
    }
...
function averageWordLength(arr){
    ...
    for (i = 0; i < arr.length; i++){
     arr[i].split(" ").length;
     averageWords = (averageWords + arr[i].split(" ").length);
    }
    return console.log(Math.round(averageWords / arr.length))
  }
